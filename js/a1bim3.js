...
const PlanosAulaModule = {
  ...
  configurarEventos() {
    ...
    // ===== NOVO: Listener para o botão de imprimir planos =====
    const btnImprimir = document.getElementById("btnImprimirPlanos");
    if (btnImprimir) {
      btnImprimir.addEventListener("click", () => this.imprimirPlanos());
    }
  },

  // ===== NOVO: Método para imprimir os planos de aula =====
  imprimirPlanos() {
    const conteudo = document.getElementById("accordionAulas").cloneNode(true);
    // Expande todos os itens para impressão
    conteudo.querySelectorAll(".accordion-collapse").forEach(el => {
      el.classList.add("show");
      el.style.display = "block";
    });
    // Remove comportamentos interativos dos botões
    conteudo.querySelectorAll(".accordion-button").forEach(btn => {
    ...
    const html = `
        <!DOCTYPE html>
        <html>
        <head><meta charset="UTF-8"><title>Planos de Aula - 1º Ano Bimestre 3</title>
        <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/css/bootstrap.min.css">
        <link rel="stylesheet" href="css/a1bim3.css">
        <style>
          /* Reforça as regras de impressão */
          body { background: white !important; padding: 20px; }
          .accordion-button { background: #f0f0e0 !important; color: #1e2a1a !important; }
          .accordion-body { background: white !important; color: #1e2a1a !important; }
          .accordion-item { border: 1px solid #ccc !important; margin-bottom: 10px; }
          .semana-card-completo { background: #f9f9f9 !important; }
          .table-robotica { background: white !important; color: #1e2a1a !important; }
          .bg-robocard { background: white !important; }
          .projeto-header { background: #eee !important; }
          .projeto-header h2 { color: #1e2a1a !important; }
          .carta-texto, .minuto-descricao, .semana-card-completo ul, .semana-card-completo p { color: #1e2a1a !important; }
          .material-badge { background: #eaeaea !important; color: #1e2a1a !important; }
          .tabela-criterios-semana { background: white !important; }
          .tabela-criterios-semana th, .tabela-criterios-semana td { border-color: #888 !important; color: #1e2a1a !important; }
          .minuto-item { background: #f0f0f0 !important; border-left-color: #666 !important; }
          .minuto-tempo { background: #ddd !important; color: #1e2a1a !important; }
          .frase-do-dia { display: none !important; }
          .check-concluido { display: none !important; }
          .badge-projeto { display: none !important; }
        </style>
        </head>
        <body>
          <div class="container">
            <h2 class="mb-4">📚 Planos de Aula Detalhados - Bimestre 3</h2>
            ${conteudo.outerHTML}
          </div>
          <script>
    ...
    if (win) {
      ...
      alert('⚠️ Pop-up bloqueado! Permita pop-ups para este site.');
    }
  }
};
...
// ========== MÓDULO JOGO: ROBÔ DAS REPETIÇÕES E DECISÕES ==========
const RoboDecisoes = {
  faseAtual: 1,
  algoritmo: [],
